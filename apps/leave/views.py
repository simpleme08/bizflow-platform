...
def _eligible(employee, start_date=None):
    if not employee.is_active or employee.status in INELIGIBLE_LEAVE_STATUSES:
        return False
    if employee.separation_date and start_date and start_date >= employee.separation_date:
        return False
    return True
...
def leave_api(request):
    ...
    if request.method == 'GET':
        ...
        return JsonResponse({'applications': [{'id': str(application.id), 'employee': f'{application.employee.first_name} {application.employee.last_name}', 'leave_type': application.leave_type.name, 'start_date': application.start_date.isoformat(), 'end_date': application.end_date.isoformat(), 'total_days': str(application.total_days), 'status': application.status} for application in applications]})
...
def leave_decision(request, application_id):
    ...
    try:
        ...
        if payload.get('decision') == 'approve':
            if not _eligible(application.employee, application.start_date) or (application.employee.separation_date and application.end_date >= application.employee.separation_date):
