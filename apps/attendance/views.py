...
def _validate_clock_photo(uploaded):
    ...
    try: uploaded.seek(0); image = Image.open(uploaded); image.verify(); uploaded.seek(0)
...
def _clock_state(employee):
    ...
    return {'employee_name':f'{employee.first_name} {employee.last_name}','employee_number':employee.employee_number,'last_action':last_action,'last_action_at':last_action_at.isoformat() if last_action_at else None,'attendance_date':today.isoformat(),'shift_name':assignment.shift_template.name if assignment else (latest_record.clock_shift.name if latest_record and latest_record.clock_shift_id else None),'can_clock_in':open_record is None and employee.is_active,'can_clock_out':open_record is not None,'assignment_available':assignment is not None,'clock_in_mode':latest_record.clock_in_mode if latest_record else None,'clocking_note':'Scheduled shift' if assignment else 'No scheduled shift. You may still clock in for a cover or unscheduled shift; the punch will be flagged for schedule reconciliation before payroll approval.','clock_in_photo_captured':bool(open_record and open_record.clock_in_photo),'clock_out_photo_captured':bool(latest_record and latest_record.time_out and latest_record.clock_out_photo)}
...
def clock_action(request):
    ...
    if action=='CLOCK_IN':
        ...
        if assignment is None:
            ...
            if requested_mode not in (AttendanceRecord.ClockInMode.COVER, AttendanceRecord.ClockInMode.UNSCHEDULED): return JsonResponse({'detail':'No scheduled shift is assigned. Confirm this is a cover or unscheduled work shift and try again.'},status=400)
            mode=requested_mode
...
def dashboard(request):
    ...
    if membership is None: return JsonResponse({'detail':'No active organization membership found. Select an organization first.'},status=403)
...
def record_attendance(request):
    if not request.user.is_authenticated: return JsonResponse({'detail':'Authentication credentials were not provided.'},status=401)
    membership=current_membership(request)
    if membership is None or not membership.has_permission('manage_attendance'): return JsonResponse({'detail':'No active selected organization with attendance-management access.'},status=403)
    try:
...
def timekeeping_template(request):
    if not request.user.is_authenticated: return JsonResponse({'detail':'Authentication credentials were not provided.'},status=401)
    membership=current_membership(request)
    if membership is None or not _can_import_timekeeping(request,membership): return JsonResponse({'detail':'HR or Super User access is required for the selected organization.'},status=403)
