...
def billing_checkout(request):
    ...
    try:
        ...
        existing = BillingSubscription.objects.filter(organization=organization, status__in=['incomplete', 'active', 'past_due', 'unpaid']).order_by('-created_at').first()
        ...
        subscription, _ = BillingSubscription.objects.update_or_create(
            ...
            defaults={'organization': organization, 'provider_customer_id': customer_id, 'provider_plan_id': plan_id, 'plan_code': plan_code, 'status': status, 'current_period_end': attrs.get('next_billing_schedule'), 'canceled_at': None, 'metadata': attrs},
        )
        ...
        AuditEvent.objects.create(organization=organization, actor=request.user, action='BILLING_CHECKOUT_STARTED', entity_type='BillingSubscription', entity_id=subscription.id, details={'plan_code': plan_code, 'provider': 'paymongo', 'provider_subscription_id': subscription_id})
        return JsonResponse({'provider': 'paymongo', 'plan_code': plan_code, 'amount_php': str(plan.monthly_price), 'subscription_id': subscription_id, 'subscription_status': status, 'invoice_id': invoice.provider_invoice_id if invoice else None, 'payment_intent_id': payment_intent_id, 'payment_intent_client_key': client_key, 'public_key': getattr(settings, 'PAYMONGO_PUBLIC_KEY', ''), 'next_step': 'Complete the first invoice payment using the returned payment intent client key.' if client_key else 'Complete payment using the latest PayMongo invoice/payment flow.'})
...
def billing_change_plan(request):
    ...
    subscription = BillingSubscription.objects.filter(organization=membership.organization, status__in=['incomplete', 'active', 'past_due', 'unpaid']).order_by('-created_at').first()
...
def billing_invoices(request):
    ...
    return JsonResponse({'invoices': [{'id': str(item.id), 'provider_invoice_id': item.provider_invoice_id, 'subscription_id': item.subscription.provider_subscription_id if item.subscription else None, 'amount': str(item.amount), 'currency': item.currency, 'status': item.status, 'due_date': item.due_date.isoformat() if item.due_date else None, 'paid_at': item.paid_at.isoformat() if item.paid_at else None, 'description': item.description} for item in invoices]})
...
def paymongo_webhook(request):
    ...
    with transaction.atomic():
        webhook, created = BillingWebhookEvent.objects.get_or_create(provider_event_id=event_id, defaults={'event_type': event_type, 'livemode': bool(attrs.get('livemode')), 'payload': payload})
        if not created:
        ...
        if subscription:
            status_map = {'subscription.activated': 'active', 'subscription.past_due': 'past_due', 'subscription.unpaid': 'unpaid', 'subscription.cancelled': 'cancelled', 'subscription.canceled': 'cancelled', 'subscription.updated': resource_attrs.get('status')}
            ...
            if new_status:
                ...
                org.subscription_status = {'active': 'ACTIVE', 'past_due': 'PAST_DUE', 'unpaid': 'PAST_DUE', 'cancelled': 'CANCELED'}.get(new_status, org.subscription_status)
            ...
            if event_type.startswith('subscription.invoice.'):
                payment_intent = resource_attrs.get('payment_intent') or {}
                payment_intent_id = payment_intent.get('id', '') if isinstance(payment_intent, dict) else payment_intent
                provider_invoice_id = resource.get('id', '')
                if provider_invoice_id:
                    invoice, _ = BillingInvoice.objects.update_or_create(provider_invoice_id=provider_invoice_id, defaults={'organization': subscription.organization, 'subscription': subscription, 'provider': 'paymongo', 'provider_payment_intent_id': payment_intent_id or '', 'amount': (resource_attrs.get('amount') or 0) / 100, 'currency': resource_attrs.get('currency', 'PHP'), 'status': resource_attrs.get('status', 'open'), 'due_date': resource_attrs.get('due_date'), 'description': resource_attrs.get('description', ''), 'metadata': resource_attrs})
