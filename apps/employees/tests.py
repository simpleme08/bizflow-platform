...
from .models import Employee, EmployeeAssignment, EmployeeDocument, EmployeeEmploymentHistory
...
class EmployeeDirectoryTests(TestCase):
    ...
    def test_manager_cannot_change_lifecycle(self):
        self.client.login(username='manager', password='password')
        response = self.client.post(f'/api/employees/{self.employee.id}/lifecycle/', data='{"action":"REGULARIZE"}', content_type='application/json')
        self.assertEqual(response.status_code, 403)

    def test_employee_self_can_view_private_profile(self):
...
class EmployeeLifecycleTests(TestCase):
    ...
    def test_regularization_creates_history_and_sets_date(self):
        self.client.login(username='hr', password='password')
        response = self.client.post(f'/api/employees/{self.employee.id}/lifecycle/', data='{"action":"REGULARIZE","effective_date":"2026-09-01","reason":"Passed probation"}', content_type='application/json')
        self.assertEqual(response.status_code, 200)
        ...
        self.assertEqual(EmployeeEmploymentHistory.objects.filter(employee=self.employee, action='REGULARIZE').count(), 1)

    def test_separation_deactivates_employee(self):
        self.client.login(username='hr', password='password')
        response = self.client.post(f'/api/employees/{self.employee.id}/lifecycle/', data='{"action":"TERMINATE","effective_date":"2026-09-10","reason":"End of employment"}', content_type='application/json')
        self.assertEqual(response.status_code, 200)
        self.employee.refresh_from_db()
    ...
    def test_other_organization_is_hidden(self):
        other = Organization.objects.create(name='Other', slug='other')
        other_user = get_user_model().objects.create_user(username='otheremployee')
    ...
    def test_document_expiry_flags(self):
        document = EmployeeDocument.objects.create(employee=self.employee, document_type='GOVERNMENT_ID', name='ID', expiry_date='2099-01-01')
        self.assertFalse(document.is_expired)
        self.assertFalse(document.expires_soon)
