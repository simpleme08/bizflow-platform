...
class PayrollCalculator:
	...
	def calculate(cls, employee, period, other_deductions=Decimal('0.00')):
		...
		return {
			'basic_pay': basic_pay, 'overtime_pay': overtime_pay, 'late_deduction': late_deduction,
			'undertime_deduction': undertime_deduction, 'other_deductions': other_deductions,
			'gross_pay': gross_pay, 'net_pay': money(gross_pay - late_deduction - undertime_deduction - other_deductions),
		}

	@classmethod
	def process_period(cls, period, organization):
		employees = employee_queryset = organization.employees.filter(is_active=True).select_related('salary')
		processed = 0
