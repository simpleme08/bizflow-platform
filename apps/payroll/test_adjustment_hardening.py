...
class PayrollAdjustmentApprovalHardeningTests(TestCase):
    def setUp(self):
        ...
        OrganizationMembership.objects.create(user=self.user, organization=self.organization, role=OrganizationMembership.Role.HR, is_active=True)
        self.employee = Employee.objects.create(
            organization=self.organization, user=self.employee_user, employee_number='EMP-ADJ-001',
            first_name='Test', last_name='Employee', is_active=True,
        )
        self.period = PayrollPeriod.objects.create(
            organization=self.organization, name='Adjustment Test Period', start_date='2026-09-01',
            end_date='2026-09-15', status=PayrollPeriod.Status.CALCULATED,
        )
        self.record = PayrollRecord.objects.create(
            employee=self.employee, payroll_period=self.period, status=PayrollRecord.Status.DRAFT,
    ...
    def test_adjustment_cannot_be_created_as_approved(self):
        response = self.client.post(
            ...
            {'kind': PayrollAdjustment.Kind.EARNING, 'description': 'Manual earning', 'amount': '500.00', 'taxable': 'true', 'approved': 'true'},
        )
