...
def payroll_api(request):
	...
	records = PayrollRecord.objects.filter(employee__organization=membership.organization).select_related('employee', 'payroll_period')
...
def process_payroll(request):
	...
	try:
		period = PayrollPeriod.objects.get(id=request.POST.get('period_id'))
...
def approve_payroll(request, record_id):
	...
	if membership is None:
		return JsonResponse({'detail': 'Payroll management permission is required.'}, status=403)
	try:
		record = PayrollRecord.objects.get(id=record_id, employee__organization=membership.organization)
